      ...
      <div>
        {products.map((product) =>
          <div className="product" key={product.product.id}>
            ...
            <h3 data-testid="shopping-cart-product-name" className="product-name">{product.product.title}</h3>
            <ChangeQnt
              qnt={product.qnt}
        ...
        <Link to="/checkout" data-testid="checkout-products">
          <button className="end-shopping" type="button" onClick={this.handleLocalStorage}>Finalizar Compra</button>
        </Link>
      </div>
    );
  }
